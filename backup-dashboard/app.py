...
def manual_backup(vmid, storage):
    ...
    # Proxmox expone el backup manual en /nodes/{node}/vzdump para ambos
    # tipos de invitado; el VM/CT se indica mediante el parámetro vmid.
    task = pve_manage(f"/nodes/{NODE}/vzdump", "POST", {**values, "vmid": vmid})
...
HTML = HTML.replace('</style>', '.schedule-menu{position:relative;display:inline-block;z-index:1}.schedule-menu[open]{z-index:100}.schedule-menu summary{list-style:none;cursor:pointer}.schedule-menu summary::-webkit-details-marker{display:none}.schedule-status{display:inline-block;border-radius:99px;padding:4px 8px;font-size:12px;font-weight:700;white-space:nowrap}.schedule-good{background:#102b25;color:var(--ok)}.schedule-bad{background:#4a1f2c;color:#ffb4c0}.schedule-popup{position:absolute;right:0;top:30px;z-index:100;min-width:320px;padding:8px;background:var(--panel);border:1px solid var(--line);border-radius:10px;box-shadow:0 10px 30px #0008}.schedule-line{display:flex;align-items:center;justify-content:space-between;gap:8px;margin:4px 0}.manual-button{border:0;border-radius:7px;background:var(--blue);color:#071321;font-weight:700;cursor:pointer;padding:4px 8px}.manual-button:disabled{opacity:.45;cursor:wait}table{overflow:visible}</style>', 1)
